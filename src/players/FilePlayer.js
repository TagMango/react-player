...
export default class FilePlayer extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (this.props.url !== prevProps.url && !isMediaStream(this.props.url)) {
  ...
  addListeners(player) {
    ...
    player.addEventListener("fullscreenchange", this.onFullscreenChange);
  ...
  load(url) {
    const { hlsVersion, hlsOptions, dashVersion, flvVersion } =
      this.props.config;
    if (this.hls) {
      this.hls.destroy();
    }
  ...
  render() {
    const { url, playing, loop, controls, muted, config, width, height } =
      this.props;
